...
Modes: default (KDE→K2H), --full-pipeline (T2KDE→K2H), --e2e-model.
...
def run_t2kde(
    ...
    with torch.no_grad():
        return (
            model(torch.from_numpy(trk).float().to(device))
            .cpu()
            .numpy()
            .astype(np.float32)
        )
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.e2e_model and args.full_pipeline:
        raise ValueError("--e2e-model and --full-pipeline are mutually exclusive.")
    ...
    if args.e2e_model:
        ...
        if args.full_pipeline:
            if not args.t2kde_model:
                raise ValueError("--t2kde-model required with --full-pipeline")
            t2kde = MaskedDNN(**T2KDE_CONFIG)
            load_ckpt(args.t2kde_model, t2kde, device)
            mode_label = "T2KDE → K2H (full pipeline)"
        else:
    ...
    if args.root:
        if not args.qibin:
            raise ValueError("--qibin required with --root")
        print("\n--- Loading ROOT Truth (nTracks >= 2 filter) ---")
        root_z, root_n, root_mu = load_root_truth(args.root)
        with open(args.qibin, "rb") as fp:
            qibin = list(pickle.load(fp))
        print(f"  qibin: {len(qibin)} entries")
    ...
    with h5py.File(args.h5, "r") as h5f:
        for i, eidx in enumerate(event_indices):
            ...
            if args.e2e_model:
                ...
                ph = run_k2h(
                    run_t2kde(h5f["tracks"][s0:s1], t2kde, device), k2h, device
                )
            else:
            ...
            for ii in range(len(p_pvs_r)):
                for jj in range(ii + 1, len(p_pvs_r)):
                    dz = float(p_pvs_r[ii] - p_pvs_r[jj])
                    pairwise_dz += [dz, -dz]
            if i < 5 or i % 50 == 0:
                print(
    ...
    try:
        popt, pcov = curve_fit(
            sigmoid_fit, ctrs, cnts.astype(float), p0=FIT_P0, maxfev=10000
        )
    ...
    results = dict(
        mode="e2e"
        if args.e2e_model
        else ("full_pipeline" if args.full_pipeline else "stage2"),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--h5", required=True, help="HDF5 file path")
    ...
    parser.add_argument(
        "--root", default=None, help="ROOT truth file (nTracks>=2 filter)"
    )
    parser.add_argument("--qibin", default=None, help="h5↔ROOT index map")
    parser.add_argument("--indices", required=True, help="Test event indices pickle")
    ...
    parser.add_argument("--full-pipeline", action="store_true", dest="full_pipeline")
