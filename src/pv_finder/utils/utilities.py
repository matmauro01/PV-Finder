...
def import_progress_bar(notebook):
    ...
    if notebook is None:
        ...
        # Force tqdm for terminal even when piping to file (e.g., with tee)
...
def load_full_state(model_to_update, Path, freeze_weights=False):
    ...
    update_dict = {
        ...
        if k in model_to_update.state_dict()
    }
    model_to_update.load_state_dict(update_dict, strict=False)
    print(
        "Of the "
        + str(len(model_to_update.state_dict()) / 2)
        + " parameter layers to update in the current model, "
        + str(len(update_dict) / 2)
        + " were loaded"
    )
...
# Loading state of the neural network
def load_checkpoint(
    model,
    optimizer,
    path="/data/home/matmauro/codice/PV-Finder/model_weights/checkpoint.pth",
    ...
    # Load onto model and optimizer
    model.load_state_dict(checkpoint["model_state"])

    # HARD CODED: turned off load optimizer state
    optimizer.load_state_dict(checkpoint["optimizer_state"])

    print(f"Epoch: {checkpoint['epoch']}, Average Train Loss: {checkpoint['loss']}")
