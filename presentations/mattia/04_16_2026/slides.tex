    ...
    \begin{columns}[c]
        \begin{column}{0.55\linewidth}
            \begin{enumerate}
                ...
                \item[\textcolor{pennred}{\textbf{V.}}] \textbf{This week}: the truth\(\to\)peak transfer gap found and closed (v3)
            \end{enumerate}
        \end{column}
        \begin{column}{0.45\linewidth}
            \centering
...
\begin{frame}{TTVA IV: From Scores to Vertices}
    ...
    \begin{columns}[c]
        ...
        \begin{column}{0.45\linewidth}
            ...
            {\scriptsize\textcolor{black!55}{\(\mu\approx60\): clean-vertex efficiency vs \(t\); \(t^*=0.98\)
            gains +4.5 points over \(t=0.5\).}}\\[3pt]
...
\begin{frame}{TTVA V: Results at \(\mu\approx60\) (MC, 2550 events)}
    ...
    \begin{tabular}{lcccc}
        ...
        \hline
        PV-Finder peaks + GNN (\(t^*=0.98\)) & 80.9\% & 11.8\% & 0.9\% & 74.5\% \\
        \quad + drop-empty, \(t=0.995\) & 86.8\% & 8.2\% & 0.01\% & \textbf{77.0\%} \\
        AMVF & 76.7\% & 20.0\% & 0.7\% & 60.9\% \\
    \end{tabular}\\[3pt]
    {\footnotesize Above AMVF in \emph{every} multiplicity bin; track-level \(F_1\) 0.866 vs 0.849.}
\end{frame}
...
\begin{frame}{TTVA VI: Run 3 Collision Data (truth-free check)}
    \begin{columns}[c]
        \begin{column}{0.55\linewidth}
            ...
            \begin{itemize}
                \setlength\itemsep{0.5em}
                \item \textbf{84.6\%} of AMVF-assigned tracks land on the GNN vertex matched to their AMVF vertex
                (\textbf{92.6\%} among tracks both methods assign)
                \item Leading-\(\sum p_T^2\) (hard-scatter proxy) vertex agrees in \textbf{84.8\%} of events
                \item Sanity closure: AMVF scored against itself through the same machinery = 1.0000 exactly
            \end{itemize}
        \end{column}
        \begin{column}{0.45\linewidth}
            \begin{itemize}
    ...
    \begin{columns}[c]
        ...
        \begin{column}{0.45\linewidth}
            ...
            \begin{itemize}
                ...
                \item \textbf{GNN v3: 71.6\%} = \textbf{96\% of the oracle} -- the associator is nearly saturated;
...
\begin{frame}{TTVA VIII: The Truth\(\to\)Peak Transfer Gap (found and closed)}
    ...
    \begin{columns}[c]
        \begin{column}{0.55\linewidth}
            ...
            \end{enumerate}
            \vspace{0.25em}
            \textbf{v3 fix}: heights repaired + truth graphs made \emph{chain-like} at build time
        ...
        \begin{column}{0.45\linewidth}
            ...
            {\scriptsize\textcolor{black!55}{v3 training: smooth climb past both earlier models
    ...
    \begin{columns}[c]
        ...
        \begin{column}{0.52\linewidth}
            ...
            \begin{tabular}{lcc}
                ...
                \hline
                AMVF & 57.3\% & 0.91\% \\
                PVF + GNN v3 (\(t=0.98\)) & \textbf{71.6\%} & \textbf{0.05\%} \\
                \hline
    ...
    \begin{columns}[t]
        \begin{column}{0.5\linewidth}
            ...
            \begin{itemize}
                ...
                \item Beats AMVF end-to-end at \(\mu\approx60\) (77.0 vs 60.9) \emph{and} PU200 (71.6 vs 57.3)
        ...
        \begin{column}{0.5\linewidth}
            ...
            \begin{itemize}
                ...
                \item \(\mu\approx60\) retrain with the height fix + augmentation (same gains expected there)
